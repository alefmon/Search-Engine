...
int main() {
    ...
    server.resource["^/json$"]["POST"]=[&server](std::shared_ptr<HttpServer::Response> response, shared_ptr<HttpServer::Request> request) {
        try {
            ...
            string query = pt.get<string>("search-text");
            string model = pt.get<string>("model");

            unordered_map<int,double> accumulators; 
            string result;   
            if(model == "boolean"){
                accumulators = searcher.boolean_search(query);                
            }      
            else if(model == "vec"){
                accumulators = searcher.vectorModel_search(query);
            }
            else if(model == "pg"){
                accumulators = searcher.pageRank_search(query);
            }
            else if(model == "anchor"){
                accumulators = searcher.anchor_search(query);
            }
            else{
                accumulators = searcher.search(query);
            }
            vector<pair<int, double> > acc;
            for(auto it : accumulators){
                acc.push_back(make_pair(it.first, it.second));
            }

            sort(acc.begin(), acc.end(), cmp);

            int num_results = 20; // Show only top-20 results.
            for(auto p : acc){
                int docId = p.first;
                result += (searcher.urls[docId] + " ");
                num_results --;
                if(num_results == 0){
                    break;
                }
