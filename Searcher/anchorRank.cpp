...
bool AnchorRank::cmp(pair <int, double> a, pair<int, double> b){
  return a.second > b.second;
}

AnchorRank::AnchorRank(int num_docs, unordered_map<string, int>& vocabulary, unordered_map<int, string>& urls, 
  vector<string>& stopwords, unordered_map<int, unsigned long long>& anchor_seek){
      
      this->urls = urls;
      this->vocabulary = vocabulary;
      this->stopwords = stopwords;
      this->anchor_seek = anchor_seek;
...
bool AnchorRank::is_stopword(const string& word){
...
bool AnchorRank::indexable(const string& word){
  if(word.empty()){
    return false;
  }
...
double AnchorRank::similarity(const vector<double>& a, const vector<double>& b){
...
unordered_map<int, int> AnchorRank::get_docsList(int term_id){
  unordered_map<int, int> result;
  unsigned long long pos;
  pos = anchor_seek[term_id];
  anchor_index_file.open("Searcher/util/Anchor_index.txt");
  anchor_index_file.seekg(pos);
  string line;

  while(getline(anchor_index_file, line)){
    string delimiter = ",";
  ...
  anchor_index_file.close();
  return result;
}

unordered_map<int,double> AnchorRank::search(const string& s){
  ...
  for(auto term : qWords){
    if(vocabulary.find(term) != vocabulary.end()){
      ...
      float term_weight = 1 + log(num_docs/docsList.size());
